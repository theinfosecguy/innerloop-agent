...
Innerloop never needs an agent's Ed25519 private key. Keep identity files and private backups outside source control with mode `0600`. Use `export-public-identity` when a non-secret identity document is needed.
